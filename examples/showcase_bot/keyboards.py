...
def buttons_showcase_kb() -> InlineKeyboardMarkup:
    ...
    kb.add(
        ...
        KeyboardButton(text="PRIMARY", callback_data="btn:primary", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="ATTENTION", callback_data="btn:attention", style=StyleKeyboard.ATTENTION),
    )
    kb.row(
        KeyboardButton(text="Документация VK Teams Bot API", url="https://teams.vk.com/botapi/"),
    )
    kb.row(
        KeyboardButton(text="Одна в ряду", callback_data="btn:single", style=StyleKeyboard.PRIMARY),
    )
    kb.add(
    ...
    kb.row(
        KeyboardButton(text="Всплывающее уведомление", callback_data="btn:alert", style=StyleKeyboard.PRIMARY),
    )
    kb.row(
        KeyboardButton(text="Открыть URL по callback", callback_data="btn:url", style=StyleKeyboard.PRIMARY),
    )
    kb.row(
        KeyboardButton(text="Композиция клавиатур (+)", callback_data="btn:compose", style=StyleKeyboard.PRIMARY),
    )
    kb.row(KeyboardButton(text="<< В главное меню", callback_data="menu:main"))
    return kb
...
def nav_level4_kb(section: str, item: str) -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=1)
    kb.add(KeyboardButton(
        text="<< Назад", callback_data=f"nav:back:l3:{section}:{item}",
    ))
    return kb
...
def formatting_menu_kb() -> InlineKeyboardMarkup:
    ...
    kb.add(
        ...
        KeyboardButton(text="Списки (Format API)", callback_data="fmt:lists", style=StyleKeyboard.PRIMARY),
    )
    kb.row(KeyboardButton(text="<< В главное меню", callback_data="menu:main"))
    return kb
...
def wzb_size_kb() -> InlineKeyboardMarkup:
    ...
    for s in PIZZA_SIZES:
        kb.add(KeyboardButton(
            text=s, callback_data=f"wzb:size:{s.lower()}",
            style=StyleKeyboard.PRIMARY,
        ))
    kb.row(KeyboardButton(text="Отмена", callback_data="wzb:cancel", style=StyleKeyboard.ATTENTION))
    return kb
...
def wzb_crust_kb() -> InlineKeyboardMarkup:
    ...
    for c in PIZZA_CRUSTS:
        kb.add(KeyboardButton(
            text=c, callback_data=f"wzb:crust:{c.lower()}",
            style=StyleKeyboard.PRIMARY,
        ))
    kb.row(
        KeyboardButton(text="<< Назад", callback_data="wzb:back:size"),
        KeyboardButton(text="Отмена", callback_data="wzb:cancel", style=StyleKeyboard.ATTENTION),
    )
    return kb
...
def wzb_sauce_kb() -> InlineKeyboardMarkup:
    ...
    for s in PIZZA_SAUCES:
        kb.add(KeyboardButton(
            text=s, callback_data=f"wzb:sauce:{s.lower()}",
            style=StyleKeyboard.PRIMARY,
        ))
    kb.row(
...
def wzb_confirm_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=2)
    kb.add(
        KeyboardButton(text="Подтвердить", callback_data="wzb:confirm", style=StyleKeyboard.PRIMARY),
...
def wzt_back_cancel_kb(back_step: str | None = None) -> InlineKeyboardMarkup:
    ...
    kb.add(KeyboardButton(text="Отмена", callback_data="wzt:cancel", style=StyleKeyboard.ATTENTION))
    return kb
...
def wzt_confirm_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=2)
    kb.add(
        KeyboardButton(text="Подтвердить", callback_data="wzt:confirm", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="<< Назад", callback_data="wzt:back:phone"),
    )
    kb.row(KeyboardButton(text="Отмена", callback_data="wzt:cancel", style=StyleKeyboard.ATTENTION))
    return kb
...
def wzm_event_kb() -> InlineKeyboardMarkup:
    ...
    for e in EVENT_TYPES:
        kb.add(KeyboardButton(
            text=e, callback_data=f"wzm:event:{e.lower()}",
            style=StyleKeyboard.PRIMARY,
        ))
    kb.row(KeyboardButton(text="Отмена", callback_data="wzm:cancel", style=StyleKeyboard.ATTENTION))
    return kb
...
def wzm_attendees_kb() -> InlineKeyboardMarkup:
    ...
    kb.add(
        ...
        KeyboardButton(text="Отмена", callback_data="wzm:cancel", style=StyleKeyboard.ATTENTION),
    )
    return kb
...
def wzm_meal_kb() -> InlineKeyboardMarkup:
    ...
    for m in MEAL_PREFS:
        kb.add(KeyboardButton(
            text=m, callback_data=f"wzm:meal:{m.lower()}",
            style=StyleKeyboard.PRIMARY,
        ))
    kb.row(
...
def wzm_notes_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=2)
    kb.add(
        KeyboardButton(text="Пропустить", callback_data="wzm:skip", style=StyleKeyboard.BASE),
...
def wzm_confirm_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=2)
    kb.add(
        KeyboardButton(text="Подтвердить", callback_data="wzm:confirm", style=StyleKeyboard.PRIMARY),
...
LANGUAGES = [
    "Python", "Rust", "Go", "TypeScript", "Java", "C++",
    "Kotlin", "Swift", "Ruby", "PHP", "Scala", "Haskell",
    "Elixir", "Dart", "Lua", "Zig", "OCaml", "Julia",
]
MSEL_PAGE_SIZE = 6
...
def multiselect_kb(selected: set[str], page: int = 0) -> InlineKeyboardMarkup:
    ...
    for lang in page_items:
        ...
        lang_buttons.append(KeyboardButton(text=text, callback_data=f"msel:toggle:{lang}", style=style))
    kb.add(*lang_buttons)

    nav_buttons: list[KeyboardButton] = []
    if page > 0:
        nav_buttons.append(KeyboardButton(
            text="<< Назад", callback_data=f"msel:page:{page - 1}",
            style=StyleKeyboard.PRIMARY,
        ))
    nav_buttons.append(KeyboardButton(
        text=f"Стр. {page + 1}/{total_pages}", callback_data="msel:noop",
        style=StyleKeyboard.PRIMARY,
    ))
    if page < total_pages - 1:
        nav_buttons.append(KeyboardButton(
            text="Вперед >>", callback_data=f"msel:page:{page + 1}",
            style=StyleKeyboard.PRIMARY,
        ))
    kb.row(*nav_buttons)

    kb.row(KeyboardButton(
        text="Очистить все", callback_data="msel:clear", style=StyleKeyboard.ATTENTION,
    ))
    if selected:
        kb.row(KeyboardButton(
...
def multiselect_confirm_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=2)
    kb.add(
        KeyboardButton(text="Подтвердить", callback_data="msel:confirm", style=StyleKeyboard.PRIMARY),
...
def messages_menu_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=2)
    kb.add(
        KeyboardButton(text="Ответить", callback_data="msg:reply", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Переслать", callback_data="msg:forward", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Редактировать", callback_data="msg:edit", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Удалить", callback_data="msg:delete", style=StyleKeyboard.PRIMARY),
    )
    kb.row(KeyboardButton(text="<< В главное меню", callback_data="menu:main"))
    return kb
...
def filter_parts_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=2)
    kb.add(
        KeyboardButton(text="FileFilter", callback_data="flt:part:file", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="VoiceFilter", callback_data="flt:part:voice", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="StickerFilter", callback_data="flt:part:sticker", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="MentionFilter", callback_data="flt:part:mention", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="ReplyFilter", callback_data="flt:part:reply", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="ForwardFilter", callback_data="flt:part:forward", style=StyleKeyboard.PRIMARY),
    )
...
def filter_composite_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=1)
    kb.add(
        KeyboardButton(text="AND: ReplyFilter & FileFilter", callback_data="flt:comp:and", style=StyleKeyboard.PRIMARY),
...
def chat_ops_menu_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=1)
    kb.add(
        KeyboardButton(text="Информация о чате", callback_data="chat:info", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Действие \"печатает\"", callback_data="chat:typing", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Действие \"смотрит\"", callback_data="chat:looking", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Закрепить сообщение", callback_data="chat:pin", style=StyleKeyboard.PRIMARY),
    )
    kb.row(KeyboardButton(text="<< В главное меню", callback_data="menu:main"))
    return kb
...
def di_menu_kb() -> InlineKeyboardMarkup:
    kb = InlineKeyboardMarkup(buttons_in_row=1)
    kb.add(
        KeyboardButton(text="Синхронная зависимость", callback_data="di:sync", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Асинхронная зависимость", callback_data="di:async", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Генератор-зависимость", callback_data="di:generator", style=StyleKeyboard.PRIMARY),
        KeyboardButton(text="Annotated-зависимость", callback_data="di:annotated", style=StyleKeyboard.PRIMARY),
    )
    kb.row(KeyboardButton(text="<< В главное меню", callback_data="menu:main"))
    return kb
